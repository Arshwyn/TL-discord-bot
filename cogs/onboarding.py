...
class GSModal(discord.ui.Modal, title="Final Step: Gear Score"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            ...
            f"👤 Name: `{self.ign}`\n"
            f"⚔️ Loadout: {self.primary} & {self.secondary}\n"
            f"⭐ Gear Score: {gs}\n\n"
...
class ProfileSetupView(discord.ui.View):
    ...
    @discord.ui.select(placeholder="Select Build Focus...", options=[discord.SelectOption(label="PvE Content", value="PvE"), discord.SelectOption(label="PvP Content", value="PvP")], row=2)
...
class RoleSelectView(discord.ui.View):
    ...
    async def select_role(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        try:
            # Enforce 32 character Nickname max
            ...
            pass # Failsafe if the user is the Server Owner or a higher-ranked admin
        ...
        if role_choice == "associate":
            ...
            await interaction.response.send_message(
                f"✅ Verification partial! Role applied.\n\nNow, let's configure your **{role_choice.upper()}** build for the guild directory:", 
                view=view, 
                ephemeral=True
            )
...
class OnboardingCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.bot.add_view(OnboardingStartView()) # Registers the persistent button

    @app_commands.command(name="set_onboarding_roles", description="Configure the roles used for server onboarding")
    @app_commands.default_permissions(manage_guild=True)
    @app_commands.describe(unverified_role="Optional: A 'Guest' role that the bot will remove when they verify")
    async def set_onboarding_roles(
        self, interaction: discord.Interaction, 
        tank_role: discord.Role, dps_role: discord.Role, support_role: discord.Role, 
        associate_role: discord.Role, unverified_role: discord.Role = None
    ):
        with next(get_db()) as db:
            def set_cfg(k, v):
    ...
    async def spawn_onboarding(self, interaction: discord.Interaction):
        embed = discord.Embed(
            ...
            description="Welcome! To gain full access to the server and set up your character profile, click the button below.\n\n"
                        ...
                        "3. (Raiders) Configure your primary build weapons and gear score.",
